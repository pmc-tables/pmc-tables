...
from pmc_tables import xml_parser
...
def _iter_test_data(basename):
    ...
    xml_children = xml_parser._iter_root_children(basename + '.xml')
...
def test_finders(attribute, xml_child, xml_data_):
    fn = getattr(xml_parser, "_find_{}".format(attribute))
...
def test_parse_pubmed_xml_file(xml_file):
    data = xml_parser.parse_pubmed_xml_file(xml_file)
